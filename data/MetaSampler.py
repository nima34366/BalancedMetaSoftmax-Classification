...
class SampleLearner(nn.Module):
    ...
    def init_learner(self, img_num_per_cls):
        ...
        self.sample_per_class = (self.sample_per_class / self.sample_per_class.sum()).to(self.device)
...
class MetaSampler(Sampler):
    def __init__(self, data_source, batch_size, meta_learner):
        ...
        self.targets_onehot = nn.functional.one_hot(targets, num_classes).float().to(self.device)
